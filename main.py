...
if menu == 'Exploratory Data Analysis':
    ...
    st.image('images\model_mileage.png')
    st.markdown("<h4 style='text-align: left;'> Insights </h4>", unsafe_allow_html=True)
    st.markdown('''
...
''')
    ...
    st.image('images\mileage_fuel.png')
    st.markdown("<h4 style='text-align: left;'> Insights </h4>", unsafe_allow_html=True)
    st.markdown('''
 ...
 t  rucks, buses, and off-road vehicles, due to higher greenhouse gas emissions.''')
    ...
    def predict_price_decision(model, _year, engineSize, transmission, fuel):
        try:
            model_index = model_list.index(model.lower())[0][0]
            transmission_index = transmission_list.index(transmission.lower())[0][0]
            fuel_index = fuel_list.index(fuel.lower())[0][0]
        except IndexError:
            model_index = -1
            fuel_index = -1
